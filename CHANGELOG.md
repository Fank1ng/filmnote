...
### 搜索页确认流程重构
- **添加评价改为搜索确认页**：主导航入口改为“搜索”，搜索结果点击后填入只读确认卡，展示海报、标题、年份、类型、简介、导演/主创、TMDb 评分等信息
- **清单与评价动作收敛**：`加入想看 / 加入下次看 / 添加评价` 移入确认卡底部，作用于当前选中的电影或剧集，搜索结果列表选中后自动收起
- **评分弹窗承接评价**：评分、短评和剧集分季评分统一移入评价弹窗，搜索页不再直接展示评分表单
- **搜索详情补齐**：电影复用现有详情缓存补导演、简介、片长等信息，剧集通过 TMDb TV detail/credits 补主创、季数和简介

## 2026-05-18

### Couple 分栏与媒体类型清单升级
- **添加评价搜索清单操作升级**：TMDB 搜索结果中电影/剧集均支持“填入评价 / 加入想看 / 加入下次看”，加入清单时会先把该条目填入评价表单
...
- **想看清单移入影单页**：想看不再出现在主导航或用户菜单，在影单页内与“我的影单”同级切换，支持移除和快速评分
- **添加评价搜索新增操作**：TMDB 搜索结果支持“填入评价 / 加入想看”，绑定 Couple 后额外支持“加入下次看”
- **发现页支持下次看**：发现卡片保留加入想看，绑定 Couple 后额外可加入共享“下次看”队列
- **Couple 页面首版**：显示绑定状态、评分默契度、偏好对比、分歧点、双人推荐和共享下次看队列
- **下次看队列**：双方可添加、移除、上移/下移排序、快速评分；评分后自动从共享队列移除
- **双人推荐 Worker 模式**：`/recommend` 支持 `mode: "couple"` 和 `partnerUserId`，过滤双方已评分电影，推荐缓存升级到 `v4` 并区分单人/双人推荐
- **Supabase 升级脚本**：新增 `supabase_couple_upgrade_2026_05.sql`，包含 `couples`、`couple_watch_queue`、1 对 1 约束与 RLS 策略
...
### 电影详情数据链路
- **统一详情模型**：简介、导演、演员、类型、TMDB 评分、片长、海报、原名统一缓存为 normalized movie detail，详情弹窗、保存回填、批量预取共用同一结构
- **修复简介加载卡死**：旧缓存只有导演/演员或只有简介时不再被误判为完整详情，会继续补拉缺失字段
- **保存逻辑补齐详情**：从 TMDB 搜索或发现页评分保存时，会优先使用本地详情缓存写入简介/导演/海报/年份，并后台补齐缺失字段
